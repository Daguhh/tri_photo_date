...
def compute(progbar=cli_progbar, counterWdg=False, loop_stop=None):
    ...
    list_files_params = {
        ...
        'cameras' : cameras,
        'recursive' : recursive,
        'dup_mode' : duplicate_ctrl_mode if is_control_hash else False
    }
    ...
    with ImageMetadataDB() as db:
        ...
        if loop_stop is not None and loop_stop.stop_signal:
            return

        ############ Try to group by day floating ##########
        if is_group_floating_days :

            progbar.update(0, "Grouping images by date ...")
